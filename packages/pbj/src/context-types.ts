...
type FlatInvoke<
  TValue,
  TFn extends Fn<TValue> = Fn<TValue>,
  TCon extends Constructor<TValue> = Constructor<TValue>,
> =
  | readonly [TCon, ...ToInject<ConstructorParameters<TCon>>]
  | readonly [TFn, ...ToInject<Parameters<TFn>>];
type FnInvoke<TValue, TFn extends Fn<TValue> = Fn<TValue>> = readonly [
  TFn,
  ...ToInject<Parameters<TFn>>,
];
...
> =
  ...
  | (TKey extends PBinJKeyType<infer TValue>
      ...
      : TKey extends keyof TRegistry
        ? FlatInvoke<TRegistry[TKey]>
        : TKey extends Constructor
          ? ToInject<ConstructorParameters<TKey>>
          : TKey extends Fn
            ? ToInject<Parameters<TKey>>
            : never);
...
export interface RegisterContext<TRegistry extends RegistryType> {
  ...
  register<T extends keyof TRegistry, TCon extends Constructor<TRegistry[T]>>(
    key: T,
    fn: TCon,
    ...args: ToInject<ConstructorParameters<TCon>>
  ): ServiceDescriptorI<TRegistry, TRegistry[T]>;
  ...
  register<T, TKey extends PBinJKeyType<T>, TFn extends Fn<T>>(
    key: TKey,
    fn: TFn,
    ...args: ToInject<Parameters<TFn>>
  ): ServiceDescriptorI<TRegistry, T>;
  register<T, TKey extends PBinJKeyType<T>, TCon extends Constructor<T>>(
    key: TKey,
    fn: TCon,
    ...args: ToInject<ConstructorParameters<TCon>>
  ): ServiceDescriptorI<TRegistry, T>;
