...
describe("Direct pbjKey usage in context.register", () => {
  ...
  it("should handle initialization with direct pbjKeys", () => {
    ...
    expect(api.makeRequest()).toEqual(
      "Making request with API key: secret-key-123",
    );
    ...
    expect(updatedApi.makeRequest()).toEqual(
      "Making request with API key: new-secret-key",
    );
  });
