...
const props = defineProps<{
    ...
    level?: LogLevel,
    maxSize?: number,
    onConfig?: ((conf:{search:string, level?:LogLevel, maxSize?:number}) => void),
}>();
...
<v-spacer></v-spacer>
      ...
      <v-btn icon :title="props.connected ? 'Connected' : 'Disconnected'" size="small" >
             <v-icon :color="colors.blue.accent3" v-if="props.connected">mdi-access-point</v-icon>
             <v-icon :color="colors.red.accent3" v-else>mdi-access-point-off</v-icon>
      </v-btn>
      <v-btn icon>
        <v-icon :color="levelToIcon[levelRef]?.color">{{ levelToIcon[levelRef]?.icon }}</v-icon>
              <v-menu activator="parent">
            <v-list density="compact" >
                <v-list-item  @click="levelRef = l as any" v-for="[l, entry] in Object.entries(levelToIcon)">
                    <v-list-item-title>
