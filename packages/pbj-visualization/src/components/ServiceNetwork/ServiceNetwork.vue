...
import {parseGraphRawData, graphOptions, graphNodes, graphEdges, graphFilterNodeId, updateGraphDrawerData, toggleGraphDrawer, cleanupGraphRelatedStates } from './graph'
...
function mountNetwork() {
  ...
  const network = networkRef.value = new Network(node, {
    nodes: graphNodes,
    edges: graphEdges,
  }, graphOptions.value)
