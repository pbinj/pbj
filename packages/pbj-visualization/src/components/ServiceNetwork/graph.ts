...
const moduleReferences = new Map<string, { path: string, displayPath: string, mod: ServiceI }[]>()
    ...
    node.forEach((n) => {
        ...
        n.deps.forEach((dep) => {
            ...
            if (node && checkIsValidModule(node.mod)) {
                ...
                allEdges.set(`${n.fullId}-${node.mod.name}`, getEdge(node.mod.name, n.fullId))
...
export function parseGraphRawData(modules: ServiceI[], root: string) {
    ...
    modules.forEach((mod) => {
        ...
        if (totalNode.some(node => node.id === mod.name)) {
            ...
            nodeData.node.size = determineNodeSize(nodeData.edges.length + mod.dependencies.length)
            const edges: Edge[] = []
            const uniqueDeps = getUniqueDeps(mod.dependencies, (dep) => {
                edges.push(getEdge(mod.name, dep))
            })
            const incrementalDeps = uniqueDeps.filter(dep => !nodeData.mod.dependencies.includes(dep))
        ...
        const uniqueDeps = getUniqueDeps(mod.dependencies, (dep) => {
            ...
            const displayPath = removeRootPath(path)
            const isExist = !!(moduleReferencesValue.find(item => item.path === path && item.displayPath === displayPath && item.mod.name === mod.name))
            if (isExist)
                return
...
function recursivelyGetGraphNodeData(nodeId: string, depth = 0): GraphNodesTotalData[] {
    ...
    node.mod.dependencies.forEach((dep) => {
        const node = modulesMap.get(dep)
        if (node)
            result.push(...recursivelyGetGraphNodeData(node.mod.name, depth))
    })
    // unique result
    return result.reduce<GraphNodesTotalData[]>((prev, node) => {
        if (!prev.some(n => n.mod.name === node.mod.name))
            prev.push(node)
        return prev
    }, [])
}
